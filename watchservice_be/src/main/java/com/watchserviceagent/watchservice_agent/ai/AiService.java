...
public class AiService {
    ...
    public AiResult requestAnalysis(AiPayload payload) {
        try {
            ...
            // ✅ WARNING/DANGER일 때만 Gemini 호출, SAFE는 고정 문구, 실패 시 fallback
            String guidance = geminiAdviceService.guidanceFor(payload, result);
            result = AiResult.builder()
                    .label(result.getLabel())
                    .score(result.getScore())
                    .detail(result.getDetail())
                    .topFamily(result.getTopFamily())
                    .isRansomware(result.getIsRansomware())
                    .guidance(guidance)
                    .build();
