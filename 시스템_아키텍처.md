...
graph TB
    ...
    subgraph AI_LAYER ["🤖  AI 레이어"]
        FASTAPI["🐍 FastAPI\n──────────\nPython · Uvicorn\nREST 서버 :8001"]
        XGB["📊 XGBoost\n──────────\n이진 분류 모델\n9개 피처 → DANGER 판정"]
    end

    subgraph CLOUD ["☁️  외부 서비스 / 클라우드"]
    ...
    %% 클라이언트 내부
    ELECTRON -->|"java -jar 실행"| SPRING
    ELECTRON -->|"WebView 렌더링\nlocalhost:8080"| REACT

    %% 프론트 ↔ 백엔드
    REACT <-->|"REST API (HTTP)\nGET·POST·DELETE"| SPRING

    %% 백엔드 ↔ DB
    SPRING <-->|"Spring JDBC"| SQLITE

    %% 백엔드 → AI
    SPRING -->|"POST /api/analyze\n9개 피처 전송"| FASTAPI
    FASTAPI -->|"predict_proba()"| XGB
    XGB -->|"랜섬웨어 확률 반환"| FASTAPI
    FASTAPI -->|"SAFE·WARNING·DANGER"| SPRING

    %% 백엔드 → 외부
    SPRING -->|"HTTPS 요청\nDANGER·WARNING 시"| GEMINI
    SPRING -->|"SMTP 443/587\nDANGER 탐지 시"| SMTP
